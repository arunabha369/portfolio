...
const BlogEditor = () => {
    ...
    // Array field handlers (images, code) - Simplified to assume single item arrays for now based on UI, 
    // but schemas support arrays. Let's start with single text inputs for simplicity that split by newline if needed?
    // Actually, let's keep it simple: One code block, one image per section roughly?
    // For arrays, let's stick to simple textarea where lines = items?

    // Let's implement specific helpers:
    const handleArrayFieldChange = (index, field, value) => {
        // We will store as array, logic handled on save?
        // Or store as array in state?
        // Let's treat code/image fields as strings in the UI state for easier editing, and split on save
        // But wait, existing data comes as array.
        // Let's parse on load.
    };

    // Actually, let's change logic:
    // Sections state will look like mapped objects.
    // For images/code, we'll use a specific input. If user wants multiple, maybe just allow one per section for now
    // to keep UI clean, or comma separated?
    // Let's go with Comma Separated for Images, Single Block for Code.
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // 2. Handle Sections
            // First, if existing bloh, maybe delete all sections and recreate? 
            // Or careful upsert? Deleting all and recreating is easiest for order management but changes IDs.
            // Let's try "Delete all for this blog, then Insert" strategy for reliability of order.
            // (Note: In production with comments linked to sections, this is bad. But comments are linked to BLOG, so it's safe!)
            ...
            const sectionsPayload = sections.map((s, idx) => ({
                ...
                heading: s.heading,
                content: s.content,
                image: Array.isArray(s.image) ? s.image : (s.image ? [s.image] : []), // Ensure array
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    {sections.map((section, index) => (
                        <div key={index} style={{ backgroundColor: '#111', borderRadius: '16px', border: '1px solid #333', overflow: 'hidden' }}>
                            ...
                            <div style={{ padding: '1.5rem', display: 'grid', gap: '1.5rem' }}>
                                ...
                                {/* Extra Media: Image & Code - Simplified to Single Fields for UI */}
                    ...
                    {sections.length > 0 && (
                        <div style={{ textAlign: 'center', marginTop: '2rem', color: '#666', fontSize: '0.9rem' }}>
                            <p>Pro Tip: You can drag and drop images or copy-paste code directly (future feature!)</p>
                        </div>
                    )}
                </div>
            </div>
